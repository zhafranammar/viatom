        ...
        <div class="max-w-full mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <div class="mt-4">
                    <a href="/cms/materi" class="bg-green-500 hover:bg-green-700 text-white left-0 font-bold py-2 px-4 rounded">Kembali</a>
                    </div>
                    ...
                    @endif
                    <form action="{{ route('cms.materi.update', ['id' => $materi->id]) }}" method="POST">
                      @csrf
                      @method('PUT')
                      ...
                      <div>
                        <div class="form grup mt-4 ">
                            ...
                            <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" name="title" type="text" value="{{ $materi->title }}" placeholder="Title">
                        </div>
                        <div class="form grup mt-4 ">
                            <label class="block uppercase tracking-wide text-gray-700 text-base font-bold mb-2">Level</label>
                            <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" name="level" type="text" value="{{ $materi->level }}" placeholder="Level">
                        </div>
                        <div class="form grup mt-4 ">
                            <label class="block uppercase tracking-wide text-gray-700 text-base font-bold mb-2">Description</label>
                            <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" name="description" type="text" value="{{ $materi->description }}" placeholder="Video Url">
                        </div>
                        @if ($materi->src != null)
                        <div class="form grup mt-4 ">
                            {{-- buat button download materi sebelumnya aja --}}
                            <a href="{{ asset($materi->src) }}" class="bg-green-500 hover:bg-green-700 text-white left-0 font-bold py-2 px-4 rounded">Download Materi</a>
                        </div>
                        @endif
                        <div class="form grup mt-4 ">
                            <label class="block uppercase tracking-wide text-gray-700 text-base font-bold mb-2">Materi</label>
                            <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" name="file" type="file">
                        </div>
                        <div class="mt-4">
