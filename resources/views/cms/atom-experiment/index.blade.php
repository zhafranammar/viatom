    ...
    <aside class="max-w-62.5 ease-nav-brand z-990 fixed inset-y-0 py-4 pl-4 block w-full -translate-x-full flex-wrap items-center justify-between overflow-y-auto rounded-r-2xl border-0 bg-[#636357] p-0 antialiased shadow-none transition-transform duration-200 xl:left-0 xl:translate-x-0">
      ...
      <div class="items-center block w-auto max-h-screen overflow-auto grow basis-full">
        <ul class="flex flex-col pl-0">
          ...
          <li class="mt-0.5 w-full">
            <a class="py-2.7 shadow-soft-xl text-sm ease-nav-brand my-0 mx-4 flex items-center whitespace-nowrap rounded-lg bg-[#F2DC52] px-4 font-semibold text-slate-700 transition-colors" href="/cms/atom-experiments">
              <div class="shadow-soft-2xl mr-2 flex h-8 w-8 items-center justify-center rounded-lg bg-[#636357] bg-center stroke-0 text-center xl:p-2.5">
                <span class="material-symbols-outlined" style="color: #F2DC52;">science</span>
              </div>
              <span class="ml-1 duration-300 opacity-100 pointer-events-none ease-soft">Atomic Experiments</span>
            </a>
          </li>
        ...
        <div class="max-w-full mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <h2 class="font-bold font-serif text-2xl">Atom Experiment</h2>
                    <div class="mt-4">
                        <a href="/cms/atom-experiments/create" class="bg-green-500 hover:bg-green-700 text-white left-0 font-bold py-2 px-4 rounded">Tambah Level</a>
                    </div>
                    ...
                    <div class="mt-4">
                        <table class="table-auto border-separate w-full">
                            ...
                            <tbody>
                                @foreach ($atomexperiments as $atomexperiment)
                                <tr>
                                    <td class="border px-4 py-2 font-serif text-center text-xl">{{$atomexperiment->id}}</td>
                                    <td class="border px-4 py-2 font-serif text-center text-xl">{{$atomexperiment->title}}</td>
                                    <td class="border px-4 py-2 font-serif text-center text-xl">{{$atomexperiment->level}}</td>
                                    <td class="border px-4 py-2 font-serif text-center text-xl">{{$atomexperiment->description}}</td>
                                    <td class="border px-4 py-2 font-serif text-center text-xl">{{$atomexperiment->src}}</td>
                                    <td class="border px-4 py-2 font-serif align-center text-center text-md">
                                        <div class="flex space-x-4">
                                          <a href="/cms/atom-experiments/{{ $atomexperiment->id }}" class="bg-[#636357] hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-full flex items-center justify-center transition duration-300">
                                            <span class="material-symbols-outlined" style="color: #F2DC52;">info</span>
                                          </a>
                                          <a href="/cms/atom-experiments/{{ $atomexperiment->id }}/edit" class="bg-[#636357] hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-full flex items-center justify-center transition duration-300">
                                            <span class="material-symbols-outlined" style="color: #F2DC52;">edit</span>
                                          </a>
                                          <form action="/cms/atom-experiments/{{ $atomexperiment->id }}" method="POST" class="inline-block">
